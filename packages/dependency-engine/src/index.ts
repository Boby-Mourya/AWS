...
function active(state: DesiredState, dependency: string): boolean {
  ...
  const value = (state.platform as unknown as Record<string, { enabled?: boolean }>)[dependency];
...
function ruleSelected(state: DesiredState, rule: DependencyRule): boolean {
  ...
  return !rule.whenProvider || selection.provider === rule.whenProvider;
}
  ...
  for (const rule of DEPENDENCY_RULES) {
    ...
    for (const dependency of rule.requires) {
      ...
      issues.push({
        ...
        dependency,
        message: `${rule.capability}${rule.whenProvider ? `/${rule.whenProvider}` : ''} requires ${dependency}`,
        migrations: [...(rule.migrationOptions ?? [])]
      });
    }
  }
  return issues;
...
export function dependencyGraph(state: DesiredState): Record<string, string[]> {
  ...
  for (const rule of DEPENDENCY_RULES) if (ruleSelected(state, rule)) graph[rule.capability] = [...rule.requires];
  return graph;
}
