...
export class ApiClient {
  ...
  async request<T>(path:string, options:RequestOptions<T> = {}):Promise<T> {
    ...
    const timeout = setTimeout(()=>controller.abort(new Error('request timeout')), options.timeoutMs ?? 10000);
    const externalAbort = () => controller.abort(options.signal?.reason);
    options.signal?.addEventListener('abort', externalAbort, { once:true });
    try {
      const attempts = Math.max(1, 1 + (options.retries ?? 0));
      let last:unknown;
      for (let attempt=1; attempt<=attempts; attempt++) {
        try {
          ...
          if (!response.ok) {
            ...
            throw new ApiError(response.status,envelope.error?.code ?? 'HTTP_ERROR',envelope.error?.message ?? 'Request failed',envelope.error?.requestId);
          }
          return options.validate ? options.validate(value) : value as T;
        } catch (error) {
          last=error;
          if (attempt===attempts || (!safeRetry.has(method) && !options.idempotencyKey)) throw error;
          await new Promise(r=>setTimeout(r, Math.min(1000, 100 * 2 ** (attempt-1)) + Math.floor(Math.random()*75)));
        }
      }
      throw last;
...
export const api = new ApiClient(process.env.NEXT_PUBLIC_API_BASE_URL ?? '/api');
