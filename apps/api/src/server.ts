...
app.addHook('onRequest',async(request,reply)=>{
  ...
  reply.header('x-request-id',request.platformContext.requestId).header('x-correlation-id',request.platformContext.correlationId)
  ...
  const tenantKey=request.platformContext.tenantId ?? request.ip;
  limiter.check(tenantKey);
});

app.setErrorHandler((error,request,reply)=>{
  const requestId=request.platformContext?.requestId ?? request.id;
  if(error instanceof ApiError)return reply.code(error.statusCode).send(errorEnvelope(error.code,error.message,requestId));
  if((error as {validation?:unknown}).validation)return reply.code(400).send(errorEnvelope('VALIDATION_ERROR','Request validation failed',requestId));
  request.log.error({err:{name:error.name,message:error.message},requestId},'request failed');
  return reply.code(500).send(errorEnvelope('INTERNAL_ERROR','An internal error occurred',requestId));
});

app.get('/health',async()=>({status:'HEALTHY'}));
app.get('/v1/meta/capabilities',async()=>({features:{documents:true,chat:true,analytics:true,workflow:false},permissions:[]}));
...
app.post<{Body:{name:string}}>('/v1/resources',{schema:{headers:{type:'object',properties:{'idempotency-key':{type:'string',minLength:8,maxLength:200}},required:['idempotency-key']},body:{type:'object',required:['name'],properties:{name:{type:'string',minLength:1,maxLength:200}},additionalProperties:false}}},async(request,reply)=>{
  const key=String(request.headers['idempotency-key']);const cached=idempotency.get(key);if(cached)return reply.code(cached.status).send(JSON.parse(cached.body));
  const result={id:crypto.randomUUID(),name:request.body.name};idempotency.put(key,201,result);return reply.code(201).send(result);
});

if(process.env.NODE_ENV!=='test')app.listen({port:Number(process.env.PORT??4000),host:'0.0.0.0'});
export { app };
